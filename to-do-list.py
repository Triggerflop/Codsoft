...
def entertask():
    ...
    def add():
        ...
        if input_text=="":
            tkinter.messagebox.showwarning(title="Warning!",message="Please Enter task first")
        else:
            listbox_task.insert(END,input_text)
            #close the root1 window
            root1.destroy()
    root1=Tk()
    root1.title("Schedule your task")
...
#Button widget 
entry_button=Button(window,text="Add your task",width=50,command=entertask)
entry_button.pack(pady=3)

delete_button=Button(window,text="Delete task",width=50,command=deletetask)
delete_button.pack(pady=3)

mark_button=Button(window,text="Mark task as completed ",width=50,command=markcompleted)
